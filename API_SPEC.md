...
|---|---|
| Member-Point Service | `http://member-point-service:8080/internal/api/v1` |
| Battle Service | `http://battle-service:8081/internal/api/v1` |
| Market Service | `http://market-service:8082/internal/api/v1` |
| Insight-Reputation Service | `http://insight-reputation-service:8083/internal/api/v1` |
...
```
GET /internal/api/v1/battles/{battleId}/info
```
...
```json
{
  ...
  "data": {
    ...
    "description": "주말 데이트 장소 추천",
    "optionA": "성수",
    "optionB": "연남",
    "sido": "서울",
    "sigu": "성동구",
    "status": "CLOSED",
    "closedAt": "2026-05-27T18:00:00"
  },
  "timestamp": "2026-05-28T10:00:00"
}
```
...
### 4-2. Battle 투표 원본 데이터 조회

```
GET /internal/api/v1/battles/{battleId}/votes/raw
```
    ...
    "votes": [
      {
        "memberId": 1,
        "selectedOption": "A",
        "votedAt": "2026-05-27T15:30:00"
      },
      {
        "memberId": 2,
        "selectedOption": "B",
        "votedAt": "2026-05-27T16:45:00"
      }
    ]
  },
  "timestamp": "2026-05-28T10:00:00"
...
```
GET /internal/api/v1/battles/comments/{commentId}
```
...
### 5-1. Market 기본 정보 및 통계 조회 (AI 분석용)
...
### 5-2. Market 예측 참여 목록 조회 (AI 분석용)

```
GET /internal/api/v1/markets/{marketId}/insight-predictions
```
...
| size | Integer | N | 페이지 크기 (기본값: 100, 최대: 500) |
    ...
    "predictions": [
      {
        ...
        "selectedOption": "YES",
        "amount": 50.00,
        "predictedAt": "2026-05-25T10:00:00"
      },
      {
        "memberId": 2,
        "selectedOption": "NO",
        "amount": 20.00,
        "predictedAt": "2026-05-25T11:30:00"
      }
    ],
    "totalCount": 20,
    "currentPage": 0,
...
Battle → DB                      투표 저장
Battle → Member-Point            POST /internal/api/v1/points/earn          (10P 지급)
Battle → Insight-Reputation      POST /internal/api/v1/reputations/activity (활동 점수 업데이트)
...
Market → Member-Point            POST /internal/api/v1/points/spend         (Point 차감)
...
Market → Member-Point            POST /internal/api/v1/points/settlements   (정산 보상 일괄 지급)
Market → Insight-Reputation      POST /internal/api/v1/reputations/prediction (예측 정확도 업데이트)
...
Market → Member-Point            POST /internal/api/v1/points/refunds       (전액 환불)
...
Insight-Reputation               AI 분석 트리거
  → Battle                       GET /internal/api/v1/battles/{battleId}/votes/raw
  → Market (필요 시)             GET /internal/api/v1/markets/{marketId}/insight-summary
