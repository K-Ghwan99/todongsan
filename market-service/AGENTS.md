...
1. 요청 검증
   ...
   - 동일 Market 사용자 중복 참여 여부 확인

2. POINT_PENDING 예측 기록 생성 후 커밋
   - market_prediction.status = POINT_PENDING
   - point_spend_idempotency_key 저장
...
Member-Point 차감 요청의 멱등성 키는 Market과 Member 기준으로 생성한다.
...
- 동일 사용자가 같은 Market에 중복 참여 요청을 보내도 같은 멱등성 키를 사용한다.
...
| idempotencyKey | `MARKET_PREDICTION_SPEND:market:{marketId}:member:{memberId}` | Point 차감 중복 요청 방지 |
...
[ ] Point 차감 idempotencyKey가 `MARKET_PREDICTION_SPEND:market:{marketId}:member:{memberId}`인가?
