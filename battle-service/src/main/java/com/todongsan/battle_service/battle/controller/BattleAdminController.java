...
public class BattleAdminController {
    ...
    // GET /api/v1/battles/admin/pending?page=0&size=10
    @GetMapping("/admin/pending")
    public ApiResponse<Page<BattleDetailResponse>> getPendingBattles(
            @RequestHeader("X-Member-Role") String role,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        validateAdmin(role);
        return ApiResponse.ok(battleService.getPendingBattles(page, size));
    }

    // GET /api/v1/battles/admin/{battleId} — PENDING 포함 모든 상태 단건 조회
    @GetMapping("/admin/{battleId}")
    public ApiResponse<BattleDetailResponse> getAdminBattleDetail(
    ...
    @PatchMapping("/admin/{battleId}/approve")
    ...
    @PatchMapping("/admin/{battleId}/reject")
    ...
    @PatchMapping("/admin/{battleId}/cancel")
