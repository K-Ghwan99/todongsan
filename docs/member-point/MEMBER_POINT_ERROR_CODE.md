...
## 7. 정산/환불 batch item 실패 시 results[].errorCode 목록

정산/환불 API는 item별로 성공/실패를 `results[]`에 담아 반환한다.
`results[].status = FAILED`일 때 `errorCode`에 들어올 수 있는 값 목록이다.

| errorCode | 상황 | Market 처리 |
|---|---|---|
